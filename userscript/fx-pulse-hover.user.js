 ...
 *   D. 诊断层：脚本管理器菜单、本地自检页（不向网页发布内部 API）
 *
 * 设计取舍
...
(function () {
  ...
  const store = {
    get: function (key, dflt) {
      try {
        ...
        return volatileStore.has(key) ? JSON.parse(volatileStore.get(key)) : dflt;
      } catch (e) { return dflt; }
    },
    set: function (key, value) {
      try {
        if (typeof GM_setValue === 'function') GM_setValue('fxph:' + key, value);
        else volatileStore.set(key, JSON.stringify(value));
      } catch (e) { /* 忽略 */ }
    },
  };
  ...
  if (!Number.isFinite(cfg.ttlMinutes) || cfg.ttlMinutes < 1 || cfg.ttlMinutes > 1440) cfg.ttlMinutes = 30;
  ...
  function appendWarnings() {
    if (!RATES.table) {
      ...
      const w = el('div', 'warn-stale', '⚠ 缓存已过期，等待刷新成功');
      part(w, 'warn-stale');
      card.appendChild(w);
    }
  ...
  function copyCurrent() {
    ...
    } catch (e) { copyFallback(line); }
  }

  function copyFallback(line) {
    let input = card.querySelector('[data-part="copy-fallback"]');
    if (!input) {
      input = part(el('textarea'), 'copy-fallback');
      input.readOnly = true;
      input.setAttribute('aria-label', '自动复制失败，请手动复制');
  ...
  function init() {
    ...
    console.log('%c[FXPH] FX Pulse Hover ' + VERSION + ' 已加载', 'color:#56e39f',
      '模式=' + cfg.mode + ' 大小=' + cfg.size + ' 悬停=' + cfg.dwellMs + 'ms · 设置与诊断请使用脚本管理器菜单');
  }
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
      ...
      MARKERS: MARKERS, CURRENCIES: CURRENCIES, DEFAULT_TLD: TLD_CURRENCY,
      testApi: api,
    };
  }
})();
